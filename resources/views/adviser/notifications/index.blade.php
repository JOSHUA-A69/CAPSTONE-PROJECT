    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex justify-end gap-2 sm:gap-3 flex-wrap">
                <a href="{{ route('adviser.notifications.archived') }}" class="px-2.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm bg-gray-600 hover:bg-gray-700 text-white rounded-lg sm:rounded-xl transition-all duration-200 ease-in-out hover:shadow-md flex items-center gap-1.5 sm:gap-2 font-medium">
                    ...
                    @forelse($notifications as $notification)
                        <div class="notification-item rounded-lg p-4 shadow-md transition-all {{ $notification->isUnread() ? 'bg-blue-100 dark:bg-blue-900/40 border-2 border-blue-200 dark:border-blue-700' : 'bg-gray-50 dark:bg-gray-700/30 border border-gray-200 dark:border-gray-600' }}" id="notification-{{ $notification->notification_id }}">
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <div class="ml-2 sm:ml-4 flex-shrink-0 flex flex-col items-end gap-1 sm:gap-2">
                                    <button onclick="archiveNotification({{ $notification->notification_id }})" class="flex-shrink-0 text-gray-400 hover:text-red-500 transition-colors p-1 sm:p-2" title="Archive this notification">
                                    ...
                                    @if($notification->reservation_id)
                                        <a href="{{ route('adviser.reservations.show', $notification->reservation_id) }}" class="inline-flex items-center px-2 py-1.5 sm:px-3 sm:py-2 border border-transparent text-xs sm:text-sm leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none transition">
                                            View
                                        </a>
                                    @endif
                                </div>
