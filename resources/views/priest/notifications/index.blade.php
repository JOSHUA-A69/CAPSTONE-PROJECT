<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex justify-end gap-2 sm:gap-3 flex-wrap">
                <a href="{{ route('priest.notifications.archived') }}" class="px-2.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm bg-gray-600 hover:bg-gray-700 text-white rounded-lg sm:rounded-xl transition-all duration-200 ease-in-out hover:shadow-md flex items-center gap-1.5 sm:gap-2 font-medium">
                            ...
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1">
                                    <div class="flex items-center mb-2">
                                        ...
                                        @elseif($notification->type === 'System Alert')
                                ...
                                <div class="ml-4 flex-shrink-0 flex flex-col items-end gap-2">
                                    ...
                                    @if($notification->type === 'Assignment' && $notification->reservation_id)
                                        <a href="{{ route('priest.notifications.assignment', $notification->notification_id) }}" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none transition">
                                            View Details
                                        </a>
                                    @elseif($notification->reservation_id)
                                        <a href="{{ route('priest.reservations.show', $notification->reservation_id) }}" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none transition">
                                            View
                                        </a>
                                    @endif
                                </div>
    ...
    <script>
        ...
        function markAllAsRead() {
            fetch('{{ route('priest.notifications.mark-all-read') }}', {
                method: 'POST',
        ...
        function clearAllNotifications() {
            ...
            fetch('{{ route('priest.notifications.clear-all') }}', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
            })
            .then(response => response.json())
        ...
        function confirmArchive(notificationId) {
            ...
            fetch(`/priest/notifications/${notificationId}/archive`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
            })
            .then(response => response.json())
