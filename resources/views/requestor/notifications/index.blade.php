                    ...
                    @else
                        <div class="space-y-3">
                            @foreach($notifications as $notification)
                                <div class="rounded-lg p-4 shadow-md transition-all {{ is_null($notification->read_at) ? 'bg-blue-100 dark:bg-blue-900/40 border-2 border-blue-200 dark:border-blue-700' : 'bg-gray-50 dark:bg-gray-700/30 border border-gray-200 dark:border-gray-600' }}">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <p class="text-gray-900 dark:text-gray-100 mb-2 {{ is_null($notification->read_at) ? 'font-semibold' : 'font-normal' }}">{!! $notification->message !!}</p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ optional($notification->sent_at)->diffForHumans() }}</p>
                                        </div>
                                        <div class="ml-4">
