@php
    ...
    $daysWithReservations = collect($eventItems)->map(fn($e) => (int) \Carbon\Carbon::parse($e['date'])->format('j'))->unique();
    ...
    <script>
        (function(){
            ...
            function bindFilterChanges(){
                ...
                const clearBtn = document.getElementById('homeFilterClear');
                if (clearBtn) {
                    clearBtn.addEventListener('click', () => {
                        const ids = ['homeFilterService','homeFilterVenue','homeFilterStart','homeFilterEnd'];
                        ids.forEach(id => {
                            const el = document.getElementById(id);
                            if (!el) return;
