...
<div class="max-w-6xl mx-auto py-3 sm:py-6 px-2 sm:px-4">
    <h1 class="text-lg sm:text-2xl lg:text-3xl font-bold mb-3 sm:mb-4 flex items-center gap-1.5 sm:gap-2">
        ...
        <span class="hidden sm:inline">Staff Calendar</span>
    </h1>
...
@push('scripts')
<script>
    document.addEventListener('DOMContentLoaded', initializeStaffCalendar);

    function initializeStaffCalendar() {
        if (typeof window.Calendar === 'undefined') {
            setTimeout(initializeStaffCalendar, 150);
            return;
        }

        const reservationNode = document.getElementById('staff-reservations-json');
        const scheduleNode = document.getElementById('staff-schedules-json');
        const orgBookingsNode = document.getElementById('staff-org-bookings-json');
        const calendarHost = document.getElementById('staffCalendar');
