            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold">Archived Organizations</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Organizations that have been archived can be restored or permanently deleted</p>
                        </div>
                    </div>
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-500 dark:text-gray-400">
                                                    {{ $organization->deleted_at->format('M d, Y') }}
                                                </div>
                                                <div class="text-xs text-gray-400 dark:text-gray-500">
                                                    {{ $organization->deleted_at->format('h:i A') }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex justify-end gap-2">
                                                    <!-- Restore Button -->
                                                    <form method="POST" action="{{ route('staff.organizations.restore', $organization->org_id) }}"
                                                          onsubmit="return confirm('Are you sure you want to restore {{ $organization->org_name }}?');">
                                                        @csrf
                                                        <button type="submit"
                                                    ...
                                                    <form method="POST" action="{{ route('staff.organizations.force-destroy', $organization->org_id) }}"
                                                          onsubmit="return confirm('⚠️ WARNING: This will PERMANENTLY delete {{ $organization->org_name }}. This action cannot be undone. Are you absolutely sure?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                                class="inline-flex items-center px-3 py-1 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @elseif(isset($archivedOrganizations))
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No archived organizations</h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Organizations that you archive will appear here.</p>
                            <div class="mt-6">
                                <a href="{{ route('staff.organizations.index') }}"
                    ...
                    @else
                        <div class="text-center py-12">
                            <svg class="mx-auto h-16 w-16 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                            </svg>
                            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">Archive Feature Not Available</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                                The archive functionality requires a database migration. Please contact your system administrator or run: <code class="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded">php artisan migrate</code>
                            </p>
