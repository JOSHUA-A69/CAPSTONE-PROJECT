                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                                    ...
                                    @foreach($organizations as $organization)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $organization->org_name }}</div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-gray-500 dark:text-gray-300">{{ $organization->org_desc ?? '—' }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @if($organization->adviser)
                                                    <div class="text-sm text-gray-900 dark:text-gray-100">{{ $organization->adviser->full_name ?? $organization->adviser->name }}</div>
                                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ $organization->adviser->email }}</div>
                                                @else
                                                    <span class="text-xs text-red-500">No adviser</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Staff can add organizations; admins view only.</p>
                        </div>
                    @endif
                </div>
            </div>
