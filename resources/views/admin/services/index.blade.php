        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <div class="flex flex-wrap gap-2 w-full sm:w-auto">
                    <!-- Filter Tabs Styled as Buttons -->
                    <a href="{{ route('admin.services.index') }}"
                       class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors {{ !request('status') && !request('time') ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700' }}">
                        All Services
                    </a>

                    <a href="{{ route('admin.services.index', ['status' => 'pending_priest_confirmation']) }}"
                       class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors {{ request('status') === 'pending_priest_confirmation' ? 'bg-yellow-500 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700' }}">
                        
                        @if($pendingConfirmationCount > 0)
                            <span class="mr-2 px-1.5 py-0.5 bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200 text-xs font-bold rounded-full">{{ $pendingConfirmationCount }}</span>
                        @endif
                        Pending
                    </a>

                    <a href="{{ route('admin.services.index', ['time' => 'upcoming']) }}"
                       class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors {{ request('time') === 'upcoming' ? 'bg-green-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700' }}">
                        
                        @if($upcomingCount > 0)
                            <span class="mr-2 px-1.5 py-0.5 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200 text-xs font-bold rounded-full">{{ $upcomingCount }}</span>
                        @endif
                        Upcoming
                    </a>

                    <a href="{{ route('admin.services.index', ['time' => 'past']) }}"
                       class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors {{ request('time') === 'past' ? 'bg-gray-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700' }}">
                        Past
                    </a>
                </div>
                ...
                <div class="flex gap-3 w-full sm:w-auto justify-end">
                    <a href="{{ route('admin.services.declined') }}"
                       class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-red-600 dark:text-red-400 text-sm font-medium rounded-lg shadow-sm hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors">
                    ...
                    <a href="{{ route('admin.calendar.index') }}"
                       class="inline-flex items-center px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-lg shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
